...
async function addNotification(recipientId, title, message, type = "info") {
    try {
        ...
        await firebaseFns.addDoc(
            firebaseFns.collection(firestoreDB, "notifications"),
            {
        ...
        console.log("Firebase notification added.");
    } catch (error) {
...
async function loadFirebaseNotifications() {
    try {
        ...
        renderFirebaseNotifications(notifications);

        console.log(
            "Firebase notifications loaded:",
            notifications.length
        );

    } catch (error) {
        console.error(
            "Error loading Firebase notifications:",
            error
        );
    }
}
...
function renderFirebaseNotifications(notifications) {
    ...
    body.innerHTML = notifications.map(notification => {
        ...
        if (notification.type === "note") {
            ...
            icon = "📸";
        } else if (notification.type === "secret") {
            icon = "💌";
...
function setupFirebaseNotificationRealtime() {
    ...
    try {
        ...
        if (!currentUser) {
            console.log("No current user for notification realtime.");
            return;
        }
        ...
        firebaseFns.onSnapshot(
            ...
            (snapshot) => {
                ...
                console.log(
                    "Firebase notification realtime updated."
                );
            ...
            (error) => {
                ...
                console.error(
                    "Firebase realtime notification error:",
                    error
                );
        ...
        console.error(
            "Error setting up notification realtime:",
            error
        );
